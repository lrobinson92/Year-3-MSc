...
const EditTeam = ({ editTeam, updateMemberRole, removeMember, user, teams }) => {
    ...
    useEffect(() => {
        const fetchTeam = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/teams/${id}/`, { withCredentials: true });
                setFormData({ name: res.data.name, description: res.data.description });

                // Fetch team members
                const membersRes = await axios.get(`${process.env.REACT_APP_API_URL}/api/teams/${id}/users-in-same-team/`, { withCredentials: true });
